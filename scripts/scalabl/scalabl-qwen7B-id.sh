...
for dataset in winogrande_s; do
    for sample in 10; do
        ...
        for gpuid_seed in "2 0"; do
            ...
            name=$modelwrapper-$dataset-sample$sample-eps$eps-kllr$kllr-beta$beta-gamma$gamma-seed$seed-id-blob
            CUDA_VISIBLE_DEVICES=$gpuid python run/main.py --dataset-type mcdataset --dataset $dataset \
                ...
                --seed $seed \
                --evaluate \
                --wandb-name $name --wandb-project "ScalaBL-Qwen7B-id" \
...
#ori_dataset=ARC-Easy
#for dataset in ARC-Easy; do
    #for sample in 10; do
        #for gpuid_seed in "1 1"; do
            #read gpuid seed <<< "$gpuid_seed"
            ...
            #CUDA_VISIBLE_DEVICES=$gpuid python run/main.py --dataset-type mcdataset --dataset $dataset \
                ...
                #--bayes-eps $eps --bayes-beta $beta --bayes-gamma $gamma --bayes-kllr $kllr --bayes-datasetrescaling \
                #--bayes-train-n-samples 1 --bayes-eval-n-samples 1 --bayes-eval-n-samples-final $sample  \
                #--load-lora-path checkpoints/$modelwrapper/$model/$ori_dataset/$modelwrapper-$ori_dataset-sample$sample-eps$eps-kllr$kllr-beta$beta-gamma$gamma-seed$seed-id
        #done 
    #done
#done
