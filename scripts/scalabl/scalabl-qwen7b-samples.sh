...
#for dataset in obqa; do
#for dataset in obqa; do
#for dataset in obqa ARC-Challenge ARC-Easy winogrande_m obqa boolq; do
#for dataset in winogrande_s; do
    #for sample in 10; do
        #for seed in 0 1 3; do
            #name=$modelwrapper-$dataset-sample$sample-eps$eps-kllr$kllr-beta$beta-gamma$gamma-seed$seed-samples-noU
            #CUDA_VISIBLE_DEVICES=$seed python run/main.py --dataset-type mcdataset --dataset $dataset \
                ...
                #--seed $seed \
                #--evaluate \
                #--wandb-name $name --wandb-project "SVDBLoB-Qwen7B-samples" \
...
for dataset in winogrande_s; do
    for sample in 1 3 5 7 9 11 13 15 17 20; do
        for seed in 0 1 3; do
            name=$modelwrapper-$dataset-sample$sample-eps$eps-kllr$kllr-beta$beta-gamma$gamma-seed$seed-samples$sample-noU
            CUDA_VISIBLE_DEVICES=$seed python run/main.py --dataset-type mcdataset --dataset $dataset \
                --model-type causallm --model $model --modelwrapper $modelwrapper \
                --lr 1e-4 --batch-size 4 \
                --opt adamw --warmup-ratio 0.06 \
                ...
                --bayes-eps $eps --bayes-beta $beta --bayes-gamma $gamma --bayes-kllr $kllr --bayes-datasetrescaling \
                --bayes-train-n-samples 1 --bayes-eval-n-samples 1 --bayes-eval-n-samples-final $sample  \
                --load-lora-path checkpoints/$modelwrapper/$model/$ori_dataset/$modelwrapper-$ori_dataset-sample10-eps$eps-kllr$kllr-beta$beta-gamma$gamma-seed$seed-samples-noU \
                &
        done 
        wait
    done
