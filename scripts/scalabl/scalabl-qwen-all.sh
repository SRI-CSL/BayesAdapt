...
for dataset in ARC-Easy; do
    for sample in 10; do
        for seed in 0; do
            name=$modelwrapper-$dataset-sample$sample-eps$eps-kllr$kllr-beta$beta-gamma$gamma-seed$seed-noU
            CUDA_VISIBLE_DEVICES=2 python run/main.py --dataset-type mcdataset --dataset $dataset \
                ...
                --seed $seed \
                --evaluate \
                --wandb-name $name --wandb-project "SVDBLoB-llama-all-qwen32B-noU" \
