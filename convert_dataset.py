...
def main(cfg):
    ...
    dset = dataset.dset.dset['train']
    options_map = {'1': 'A', '2': 'B'}
    for i in range(len(dset)):
        user_prompt = user_template.format(
            sentence=dset['sentence'][i],
            option1=dset['option1'][i],
            option2=dset['option2'][i],
        )
        answer = options_map[dset['answer'][i]]
        messages = [
