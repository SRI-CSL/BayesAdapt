...
import ivon
from evaluate_ivon import evaluate
from evaluate import evaluate as evaluate_mean
...
def train(cfg):
    ...
    #nll_optimizer = instantiate(cfg.optim.nll_optimizer, params, ess=len(dataloader.dataset))
    nll_optimizer = ivon.IVON(
        params, 
        ess=5000*math.sqrt(len(dataloader.dataset)), 
        lr=1e-2, 
        beta2=0.9995,
        weight_decay=0.1,
        clip_radius=0.005,
    ...
    for step in trange(cfg.optim.max_train_steps, desc="Step", disable=not cfg.pbar):
        ...
        with nll_optimizer.sampled_params(train=True):
    ...
    results = evaluate(cfg, nll_optimizer, model=model, split='test')
    results = evaluate_mean(cfg, model=model, split='test')
    return model
