...
def train(cfg):
    ...
    noise_params, no_noise_params = [], []
    for n, p in model.named_parameters():
        if p.requires_grad:
            #if 'lm_head' in n or 'lora_B' in n:
            if 'lora_B' in n:
                print(n, p.shape)
                no_noise_params.append(p)
            else:
                noise_params.append(p)
    
    warmup_steps = cfg.optim.nll_scheduler.total_steps * cfg.optim.nll_scheduler.warmup_ratio
    N = len(dataloader.dataset)
    params = [
        {"params": noise_params, "add_noise": True},
        #{"params": no_noise_params, "add_noise": False, 'lr': cfg.optim.nll_optimizer.lr / N}
        {"params": no_noise_params, "add_noise": False},
    ]
    nll_optimizer = instantiate(cfg.optim.nll_optimizer, params, num_data=N, noise_warmup_steps=warmup_steps)
