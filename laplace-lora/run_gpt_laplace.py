...
def parse_args():
    ...
    # args.output_dir += f'/openbookqa/{args.model_name_or_path}_{peft_method}_{args.lora_alpha}_{args.lora_dropout}_{args.learning_rate}_{args.seed}'
...
def main(load_step):
    ...
    elif 'MMLU' in args.task_name:
        raw_dataset = load_dataset('cais/mmlu', 'all')
        if 'chem' in args.task_name:
            split = ['college_chemistry']
        elif 'phy' in args.task_name:
            split = ['college_physics']
        df = pd.DataFrame(raw_dataset['test'])
        filtered = df[df['subject'].isin(split)]
        raw_dataset = datasets.Dataset.from_pandas(filtered)
        raw_datasets = {'train': raw_dataset, 'validation': raw_dataset, 'test': raw_dataset}
    ...
    def preprocess_function(examples):
        if args.task_name == 'boolq':
            ...
            all_choices = examples['choices']
            answers = examples['answer']
            # answer_map = {0: 'A', 1: 'B', 2: 'C', 3: 'D'}
            # answers_str = [answer_map[answer] for answer in answers]
            # choices_list = [' '.join(f'{label}. {text}' for label, text in zip(answers_str, choices))]
            choices_list = []
            possible_choices = ['A', 'B', 'C', 'D']
            for four_choices in all_choices:
                x = [' '.join(f'{label}. {text}' for label, text in zip(possible_choices, four_choices))]
                choices_list.append(x)
            texts = [f"Select one of the choices that answers the following question: {question} Choices: {choices} Answer:" for question, choices in zip(questions, choices_list)]
            result = tokenizer(texts, padding=padding, max_length=args.max_length, truncation=True)
        ...
        def __init__(self, model):
            ...
            if args.task_name == 'boolq':
                try:
                    ...
                    self.id_list = [tokenizer.encode('False')[0], tokenizer.encode('True')[0]]
                ...
                try:
                    ...
                    self.id_list = [tokenizer.encode('A')[0], tokenizer.encode('B')[0], tokenizer.encode('C')[0], tokenizer.encode('D')[0]]
            elif 'ARC' in args.task_name:
                try:
                    self.id_list = [tokenizer.encode('A')[1], tokenizer.encode('B')[1], tokenizer.encode('C')[1], tokenizer.encode('D')[1]]
                except:
                ...
                try:
                    ...
                    self.id_list = [tokenizer.encode('A')[0], tokenizer.encode('B')[0]]
            elif 'MMLU' in args.task_name:
                try:
                    self.id_list = [tokenizer.encode('A')[1], tokenizer.encode('B')[1], tokenizer.encode('C')[1], tokenizer.encode('D')[1]]
                except:
    ...
    metric_dir = os.path.join(metric.data_dir, metric.experiment_id)
